...
class Game{
    ...
    clickHandler = event =>
    {
        ...
        totalClickCounter=totalClickCounter+1;    //считаем ходы
    ...
    renderYouWin()
    {
        alert('Du ist Schritte fur '+totalClickCounter+' gewonnen');
    }
    //well done
    start()
    {
    ...
    generateCard()
    {
        ...
        {
            let selectedBg;
                //В случае, если мы наполнили уже больше половины массива
                if(this.count/2 >i)
                {
                    selectedBg = cards[Math.floor(Math.random()*cards.length)];
...
document.getElementById('start_menu').onclick =function(event)
{
    event.preventDefault();
    ...
    if(name.length<3){
        alert('Имя должно содержать минимум три символа');
        return;
    }

    if(count.length<1){
        alert('Введите количество (четное)');
        return;
    }
    if(count%2!==0)
    {
